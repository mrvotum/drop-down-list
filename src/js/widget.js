export default class Widget {
    ...
    this.dropDownList__main.addEventListener("click", () => {
      if (this.listBtn.classList.length === 2) {
        ...
        if (this.dropDownList__holder.getAttribute('data-listchecked')) {
          console.log('Во, есть отмеченные');
  ...
  addCheckboxListener() {
    ...
    if (checkboxes.length !== 0) {
      ...
      this.vacancyArrId = [];
      console.log('начать заново, должен быть пустой vacancyArrId');
      console.log(this.vacancyArrContent);
      ...
      // тут можно посмотреть надписи вакансий выбранных
      console.log(vacancyArr[0].getAttribute('data-vacancy'));
  ...
  createListTitle() {
    ...
    if (this.vacancyArrContent.length > 3) {
      ...
      // Если в строку не помещается весь текст, добавить многоточие (52 - макс кол-во символов с строке)
